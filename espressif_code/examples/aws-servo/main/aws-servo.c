...
void parse_json(void* params)
{
    ...
    while(1)
    {
        ...
        {      
            ...
            if (cJSON_IsString(object) && (object->valuestring != NULL))
            {
                ESP_LOGI(TAG, "Received update request from AWS");
            }

            //iterate over update objects
            while(object)
            {
                //Update objects composed of child objects: weight & time
                item = object->child;
                
                //update dispense amount (weight)
                if(cJSON_IsNumber(item) && (item->valueint > 0))
                {
                    valid = 1;
                    dispense_amount = (int)item->valueint;
                }
                else
                {
                    valid = 0;
                ...
                /* TODO: implement time object parsing */
...
void app_main()
{
    ...
    printf("Creating JSON parsing task\n");
